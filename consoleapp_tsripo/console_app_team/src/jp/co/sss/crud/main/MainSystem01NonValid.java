...
public class MainSystem01NonValid {
	 ...
	 *
	 * <p>
	 * メニュー表示から社員削除機能までを実装し、このクラスで呼び出す。
	 *
	 * <p>
	 * BufferedReaderを用いてコンソールから必要な項目を入力する。 入力結果に基づいてEmployeeDAOからメソッドを呼び出し、処理を行う。
	 * 参照系機能のメソッドを呼び出す場合は戻り値のエンティティリストを受け取り、 mainメソッド内でデータ表示を行う。
	 *
	 * {@code
	 * 	List<Hoge> list = xxxDAO.find(); for(Hoge hoge : list){
	 * System.out.println(hoge); } }
	 *
	 * 例外処理はExceptionをキャッチする "システムエラーが発生しました"と出力し、スタックトレースを出力する。
		...
		try {
			do {
				...
				case 6:
					...
					String inputempid2 = br.readLine();
					int empid2 = Integer.parseInt(inputempid2);
					employee.setEmpId(empid2);

					employeeDAO.delete(empid2);
